	...
	<div class="row-fluid">
		...
		<div class="btn-group table-bordered">
			...
			<a href="#myModal" role="button" class="btn" data-toggle="modal"><span class="add-on"><i class="icon-search"></i></span>&nbsp;查找</a>
					...
					<a href="<%= request.getContextPath() %>/cust_info_queryByLvl?custLvl=<s:property value="custLvl"></s:property>" class="btn"> <span class="add-on"><i class="icon-repeat"></i></span>&nbsp;刷新</a>
			...
			<table class="table table-bordered">
				...
				<tbody>
					<tr>
						...
						<% if(!crmRole.equals("comm")){ %>
						<a href="<%= request.getContextPath() %>/cust_info_delete.action?custId=<s:property value="custId"></s:property>&delMode=sgl&rtnUri=<%= currentUri %>&param=<s:property value="custLvl"></s:property>" class="btn"> <span class="add-on"><i class=" icon-trash"></i></span>&nbsp;删除</a>
						<% } %>
						</td>
					</tr>
				</tbody>
			...
			if(num==0){
				...
				if(confirm(tip)){
					...
					var custIds="";
					 ...
					 document.location.href="<%= request.getContextPath() %>/cust_info_deleteMul.action?custIds="+custIds+"&delMode=mul&delMode=sgl&rtnUri=<%= currentUri %>&param=<s:property value="custLvl"></s:property>";
					return true;
				}else{
					return false;
				}
